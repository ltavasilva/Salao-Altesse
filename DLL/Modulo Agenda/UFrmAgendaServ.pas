...
type
  TFrmAgendaServ = class(TForm)
    ...
    CDS_AgendaIdCateg: TIntegerField;
...
procedure TFrmAgendaServ.CalendarioClick(Sender: TObject);
begin
  AgendaServicos('Calendario');
end;

procedure TFrmAgendaServ.ChOnlyDispClick(Sender: TObject);
begin
  AgendaServicos('OnlyDisp');
end;

Procedure TFrmAgendaServ.Conectar(User, Pass, Database: String);
Var Str:String;
Begin
  with Conexao do
  Begin
...
Procedure TFrmAgendaServ.Desconectar();
Begin
  Try
...
procedure TFrmAgendaServ.AgendaButtonClicked(Sender: TObject;
  const Button: TButtonItem);
Var TextoBotao:String;
begin
  TextoBotao:='';
  CDS_Agenda.Locate('ID;IdCateg', VarArrayOf([Button.ID,Button.Category.Index]),
    [loCaseInsensitive]);
...
procedure TFrmAgendaServ.AgendaCategoryCollapase(Sender: TObject;
  const Category: TButtonCategory);
Var Idx: Integer;
begin
...
procedure TFrmAgendaServ.FormCreate(Sender: TObject);
begin
  FrmEvento := TFrmEvento.Create(Application);
end;

procedure TFrmAgendaServ.FormShow(Sender: TObject);
begin
  Qry_Servicos.Open;
...
procedure TFrmAgendaServ.ServicosKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAgendaServ.ServicosMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  AgendaServicos('Serviços');
end;

procedure TFrmAgendaServ.AgendaServicos(Origem: String);
